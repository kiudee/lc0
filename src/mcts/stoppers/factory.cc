...
const OptionId kITUId{"immediate-use", "immediate-use",""};
const OptionId kBPBId{"book-ply-bonus", "book-ply-bonus",""};
const OptionId kSMId{"slowmover", "slowmover",""};
}  // namespace
...
void PopulateTimeManagementOptions(RunType for_what, OptionsParser* options) {
  ...
  if (for_what == RunType::kUci) {
    ...
    options->Add<FloatOption>(kITUId, 0, 10) = 1.0f;
    options->Add<FloatOption>(kBPBId, 0, 10) = 0.4f;
    options->Add<FloatOption>(kSMId, 0, 10) = 1.0f;
...
std::unique_ptr<TimeManager> MakeTimeManager(const OptionsDict& options) {
  ...
  oss << ",immediate-use=" << std::setprecision(5) << std::fixed << options.Get<float>(kITUId);
  oss << ",book-ply-bonus=" << std::setprecision(5) << std::fixed << options.Get<float>(kBPBId);
  oss << ",slowmover=" << std::setprecision(5) << std::fixed << options.Get<float>(kSMId);
  oss << ")";
